...
<body>
    <nav class="navbar navbar-expand-lg navbar-light bg-light admin-navbar-main">
      ...
      <div class="description-page-main">
<!--  -->
<?php
$descriptionId = (isset($_GET['descMovieId']) ? $_GET['descMovieId'] : '');
$descriptionQuery="SELECT * FROM tvshows WHERE id='$descriptionId'";
...
if($resultCheckDescriptionQuery>0){
    while($row=mysqli_fetch_array($runDescriptionQuery)){
        ...
        <div>
            <img src="../posters/tvshows/<?php echo $row['poster']; ?>" alt="">
            <video src="../trailers/tvshows/<?php echo $row['trailer']; ?>" controls></video>
        </div>
        <div>
            <p><?php echo $row['description']; ?></p>
            <span>
                <h6>Director: </h6>&nbsp;&nbsp;
                <h6><?php echo $row['director']; ?></h6>
            </span>
            <span>
              <h6>Initial Release: </h6>&nbsp;&nbsp;
              <h6><?php echo date("Y",strtotime($row['releasedate'])); ?></h6>
          </span>
            <span>
              <h6>Stars: </h6>&nbsp;&nbsp;
              <h6><?php echo $row['cast']; ?></h6>
          </span>
          <span>
              <h6>Genre: </h6>&nbsp;&nbsp;
