...
<head>
    ...
    <title>Reel Reviews | Add TV Shows</title>
...
<body>
    <nav class="navbar navbar-expand-lg navbar-light bg-light admin-navbar-main">
      ...
      <div class="admin-post-content-main">
        <form class="add-tv-shows-main" action="admin_addTVShows.php" method="post" enctype="multipart/form-data">
            <h2>Add TV Show</h2>
            <div>
            ...
            <div>
                <button type="submit" name="upload">Upload TV Show</button>
            </div>
        </form>
      </div>
...
if (isset($_POST['upload'])) {
    ...
    $trailerDestination = "../../trailers/tvshows/".$trailerName; 


    move_uploaded_file($posterTmp, "../../posters/tvshows/$posterName");
    move_uploaded_file($trailerTmp, $trailerDestination);
    
    $insertQuery = "INSERT INTO tvshows(title, description, releasedate, director, cast, category, poster, trailer) VALUES ('$title','$description','$releasedate','$director','$cast','$category','$posterName','$trailerName' )";
   
    $runInsert = mysqli_query($conn, $insertQuery);
    if ($runInsert) {
        echo "<script>
        Swal.fire({
          title: 'TV Show Uploaded Successfully',
          icon: 'success',
        ...
        echo "<script>
        setTimeout(function(){window.open('admin_addTVShows.php', '_self')}, 3000);
        </script>";
    }
    else {
        echo "<script>
        Swal.fire({
          title: 'TV Show Upload Failed',
          icon: 'error',
          showConfirmButton: false,
          timer: 2000
        })
        ...
        echo "<script>
        setTimeout(function(){window.open('admin_addTVShows', '_self')}, 3000);
        </script>";
    }
}
